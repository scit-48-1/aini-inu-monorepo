...
export default function DynamicMap({ center, zoom, markers, onMarkerClick, hideCircle, interactive = true, radiusKm, onMoveEnd, selectedMarkerId }: DynamicMapProps) {
  const [visualCenter, setVisualCenter] = useState<[number, number]>(center);

  const createCustomIcon = (imageUrl: string, isEmergency: boolean, isSelected: boolean) => {
    return L.divIcon({
      ...
      >
        ...
        {markers.filter((m): m is MapMarker & { lat: number; lng: number } => m.lat != null && m.lng != null).map((m) => (
          <Marker
            ...
            icon={createCustomIcon(m.image || m.thumbnail || '', !!m.isEmergency, m.id === selectedMarkerId)}
