...
export const ChatInput: React.FC<ChatInputProps> = ({ inputText, setInputText, onSend, isArchived }) => {
  if (isArchived) {
    return (
      <footer className="p-10 bg-zinc-100/50 text-center border-t border-zinc-200">
        <div className="flex flex-col items-center gap-2 opacity-30">
          ...
          <Typography variant="label" className="text-navy-900 font-black tracking-[0.2em] text-[10px] uppercase">This conversation has ended</Typography>
        </div>
      </footer>
    );
  }
    ...
    <footer className="p-6 md:p-10 bg-white border-t border-zinc-50 z-20">
      <div className="max-w-5xl mx-auto space-y-4">
        <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2">
          {['곧 도착해요! 🏃', '몽이가 좋아해요 ✨', '다음에 또 봐요 👋', '매너 최고예요 👍'].map((reply) => (
            <button 
              key={reply} 
        ...
        <div className="relative group flex items-center gap-4">
          <button className="w-14 h-14 bg-zinc-50 text-zinc-300 rounded-2xl flex items-center justify-center hover:bg-zinc-100 hover:text-navy-900 transition-all shrink-0">
            <ImageIcon size={24} />
          </button>
          <div className="flex-1 relative">
            ...
            <button 
              ...
              className={cn(
                ...
                inputText.trim() ? "bg-amber-500 text-navy-900 scale-100" : "bg-zinc-200 text-white scale-90"
              )}
            >
