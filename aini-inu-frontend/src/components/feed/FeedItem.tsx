...
import { Heart, MessageCircle, Share2, MapPin, ChevronDown, ChevronUp, MoreHorizontal, Send, Trash2, X } from 'lucide-react';
...
export const FeedItem: React.FC<FeedItemProps> = React.memo(({ post: initialPost, currentUserId, onDelete }) => {
  const [post, setPost] = useState<FeedPostType>(initialPost);
  const [isExpanded, setIsExpanded] = useState(false);
  const [isLiked, setIsLiked] = useState(initialPost?.isLiked || false);
  const [commentText, setCommentText] = useState('');
  const [comments, setComments] = useState<CommentType[]>(initialPost?.commentsList || []);
  ...
  const authorAvatar = post.author?.avatar || '/AINIINU_ROGO_B.png';

  const authorId = post?.author?.id || 'me';
  const profileLink = authorId === 'me' ? '/profile' : `/profile/${authorId}`;
  ...
  const handleLike = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      setPost((prev: any) => ({ ...prev, likes: res.likes }));
    } catch (error) {
  ...
  const handleExpand = async () => {
    if (!isExpanded && comments.length === 0 && (post.comments || 0) > 0) {
    ...
    >
      ...
      <div className={cn(
        ...
        <div className="absolute top-5 left-5 z-30">
          <div className="bg-white/95 backdrop-blur-md px-2.5 py-1 rounded-full text-[8px] font-black flex items-center gap-1 shadow-sm border border-zinc-100 uppercase tracking-widest text-zinc-900">
            <MapPin size={10} className="text-amber-500" /> {post.location || '위치 정보 없음'}
          </div>
        </div>
      </div>
        ...
        <div className="flex items-center gap-4 w-full" onClick={(e) => !isExpanded && e.stopPropagation()}>
          ...
          <div className="flex-1 min-w-0 space-y-0.5">
            <div className="flex items-center gap-2">
              ...
              <Typography variant="label" className="text-zinc-400 font-bold tracking-[0.2em]">{post.time || '방금 전'}</Typography>
            </div>
            <Typography variant="body" className={cn(
              ...
              &quot;{post.content || post.caption || '내용이 없습니다.'}&quot;
            </Typography>
          </div>
             ...
             <div className="flex items-center justify-between mt-auto">
                <div className="flex items-center gap-4">
                   <button onClick={handleLike} className={cn("flex items-center gap-2 transition-colors", isLiked ? "text-red-500" : "text-zinc-400 hover:text-red-500")}>
                      ...
                      <Typography variant="h3" className="tracking-tighter text-navy-900">{post.likes || 0}</Typography>
                   </button>
                   <div className="flex items-center gap-2 text-zinc-400">
                      <MessageCircle size={20} />
                      <Typography variant="h3" className="tracking-tighter text-navy-900">{post.comments || 0}</Typography>
                   </div>
                </div>
             </div>
          </div>
          ...
          <div className="flex items-center justify-between mb-6 shrink-0">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <Typography variant="label" className="text-zinc-400 text-[10px]">{post.author?.handle || '@anonymous'}</Typography>
              </div>
            </div>
            <div className="flex items-center gap-2">
              {isMyPost && (
          ...
          <div className="space-y-4 mb-6 shrink-0">
            ...
            <div className="flex gap-2">
              {post.tags?.map((tag: string) => (
                <span key={tag} className="text-[10px] font-bold text-amber-500 bg-amber-50 px-2 py-1 rounded-md">#{tag}</span>
              ))}
            </div>
          </div>
            ...
            {isLoadingComments ? (
              ...
              comments.map((cmt) => (
                <div key={cmt.id} className="flex gap-3 text-sm group">
                  <img src={cmt.author?.avatar || '/AINIINU_ROGO_B.png'} className="w-8 h-8 rounded-full object-cover shrink-0" alt="User" />
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-6">
                <button onClick={handleLike} className="flex items-center gap-2 group">
                  ...
                  <Typography variant="h3" className="text-navy-900">{post.likes || 0}</Typography>
                </button>
                <div className="flex items-center gap-2 text-zinc-400">
                  <MessageCircle size={20} />
                  <Typography variant="h3" className="tracking-tighter text-navy-900">{post.comments || 0}</Typography>
