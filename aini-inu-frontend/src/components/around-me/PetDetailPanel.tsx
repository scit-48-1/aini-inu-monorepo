...
export const PetDetailPanel: React.FC<PetDetailPanelProps> = ({
  ...
  return (
    <div
      className={cn(
        'absolute top-0 right-0 h-full w-[340px] z-[1100] transition-transform duration-500 ease-out flex items-start p-3 pointer-events-none',
        isOpen && pet ? 'translate-x-0' : 'translate-x-full',
      )}
    >
      {/* Panel */}
      <div className="relative max-h-full bg-white rounded-[48px] shadow-2xl border border-zinc-100 flex flex-col overflow-hidden z-10 pointer-events-auto">
        {/* Hero Image */}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5 no-scrollbar">
          {/* Personalities */}
          {pet && pet.personalities.length > 0 && (
          ...
          {/* Walk Style */}
          {pet && pet.walkingStyles.length > 0 && (
