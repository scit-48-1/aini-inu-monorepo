...
interface AIBannerProps {
  hotspot: { region: string; count: number };
  dogName: string;
}

export const AIBanner: React.FC<AIBannerProps> = ({ hotspot, dogName }) => {
  return (
      ...
      <Card variant="vibrant" className="flex-1 bg-gradient-to-r from-amber-500/10 via-amber-500/5 to-transparent p-8 flex items-center gap-8 border-amber-500/20">
        ...
        <div className="flex-1 min-w-0">
          ...
          <Typography variant="h3" className="text-2xl font-black text-navy-900 leading-tight">
            &quot;오늘은 <span className="text-amber-500">{hotspot.region}</span>에 <span className="text-amber-500">강아지 친구들</span>이 가장 많이 모여있어요! {dogName}와 함께 산책 어때요?&quot;
          </Typography>
        </div>
